...
public class MyListener extends MouseAdapter implements KeyListener, MouseMotionListener { 
	...
	public void keyPressed(KeyEvent e) {	//플레이어 이동 구현(쉬프트는 대쉬구현 쿨타임 필요)
		if (!room.stop) {
			if(room.player.motion==Motion.deify){
				if(e.getKeyChar()=='.')
					room.player.motion=Motion.Init;
				else
					Magic.name(e.getKeyChar(),room);
			}
			...
			if(room.player.motion!=Motion.deify)
			if (e.getKeyCode() == '1' | e.getKeyCode() == '2' | e.getKeyCode() == '3' | e.getKeyCode() == '4') {
				room.player.skill(e.getKeyCode());
			}
		...
		if (!room.stop)
			if (mouse.getButton() == MouseEvent.BUTTON1) {
				if (room.player.motion != Motion.Attack) {// 플레이어의 상태
					...
					bullet = new BulletOfCharacter("bullet0.png",30,room.player.getPoint(), mouse.getPoint(), room);
					room.add(bullet);
